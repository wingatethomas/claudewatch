...
class AboutPane(BasePane):
    """About pane with version info, links, and dynamic changelog."""
    ...
    def build_content(self, view: NSView, content_top: float) -> None:  # noqa: PLR0915
        ...
        y -= 18
        version_label.setFrame_(NSMakeRect(CONTENT_PADDING, y, self.card_width, 18))
        view.addSubview_(version_label)
        y -= 18
        contact_label = secondary_label("Contact: claudewatch@protonmail.com", size=11.0)
        contact_label.setFrame_(NSMakeRect(CONTENT_PADDING, y, self.card_width, 14))
        ...
        audit_log_button.setTarget_(self.delegate)
        ...
        github_button.setTarget_(self.delegate)
        github_button.setAction_(objc.selector(self.delegate.openRepo_, signature=b"v@:@"))
...
def build_about_pane(delegate: object, w: float, h: float) -> NSView:
    """Build the About pane."""
    return AboutPane(delegate, w, h).build()
