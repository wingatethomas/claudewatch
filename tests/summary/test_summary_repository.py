...
class TestGetEntry:
    ...
    def test_returns_none_when_stale(self, tmp_path):
        ...
        # No JSONL found → mtime 0 → not stale (0 > 1.0 is False) but also not matching
        # Actually: current_mtime=0, so `not current_mtime` is True → returns None
        entry = repo.get_entry("/test")
...
class TestCache:
    ...
    def test_cache_full(self, tmp_path):
        from unittest.mock import patch
        ...
        repo = _make_repo(tmp_path)
        with patch("claudewatch.backend.core.session_log.jsonl.CLAUDE_PROJECTS_DIR", str(tmp_path / "projects")):
            repo.cache_full("/test", "Title", "• bullet 1\n• bullet 2")
            entry = repo.get_entry("/test")
        assert entry is not None
        assert entry.title == "Title"
        assert "bullet 1" in entry.summary
